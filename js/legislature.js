$(document).ready(function () {
    ...
    function loadMessage() {
        ...
        $.ajax({
            dataType: "html",
            url: 'https://cors-anywhere.herokuapp.com/' + $('#urlinput').val(),
            // url: 'https://crossorigin.me/' + encodeURIComponent($('#urlinput').val()),
            // headers: { "Origin": "http://oliver-whyte.com/waleg/" },
            success: function (data) {
                ...
                $('div#container').html($(data).find('#bulletin_body'));
                $('div#container').filter(function () { //.load(proxyurl+' #bulletin_body', function(response, status, xhr) {
                    ...
                    $rows.each(function (rownum) {
                        ...
                        if ($cells.eq(0).attr('colspan')) {
                            ...
                            if (bits && bits.length >= 3) { titlestr = "(" + bits[2] + ")" + " " + bits[1].trim(); }
                            ...
                            if (eventid != "") {
                                // Get details
                                var $detailstable = $('a[name=' + eventid + ']').next('table');
                                console.assert($detailstable.find('tr').length == 2);
                                var $detailsrow = $detailstable.find('tr').eq(1);
                                console.assert($detailsrow.find('td').length == 1);
                                var $detailsnode = $detailsrow.find('td').eq(0).clone();
                                ...
                                $detailsnode.contents().filter('ol').each(function () {
                                    $(this).text(ol2txt(this, 0, 1, "\n"));
                                });
                                // Replace <br> elems in details with newlines
                                $detailsnode.contents().filter('br').each(function () {
                                    $(this).replaceWith("<span></span>").text("\n");
                                });
                                theevent.details = $detailsnode.contents().text();
